...
def impl(context):
    with dbconn.connect(dbconn.DbURL()) as conn:
        rows = dbconn.execSQL(conn, "SELECT name, setting FROM pg_settings WHERE name LIKE 'lc_%'").fetchall()
...
def impl(context):
    locale = get_en_utf_locale()
    context.execute_steps(u'''When a demo cluster is created using gpinitsystem args "--lc-ctype=%s"''' % locale)
